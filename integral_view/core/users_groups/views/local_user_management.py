...
from integral_view.core.users_groups.forms import local_user_forms

from integralstor import local_users, audit, django_utils, config
    ...
    try:
        if request.method == "GET":
            ...
            if form.is_valid():
                ...
                #url = '/users_groups/view_local_groups?ack=created'
                url = '/users_groups/update_group_membership?grpname=%s&ack=created' % cd['grpname']
                return django.http.HttpResponseRedirect(url)
            else:
                return_dict["form"] = form
                return django.shortcuts.render_to_response("create_local_group.html", return_dict, context_instance=django.template.context.RequestContext(request))
...
def delete_local_user(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            url = '/users_groups/view_local_users?ack=deleted'
            return django.http.HttpResponseRedirect(url)
    except Exception, e:
        return_dict['base_template'] = 'users_groups_base.html'
  ...
  try:
    ...
    if request.method == "GET":
      ...
      if form.is_valid():
        ...
        return django.http.HttpResponseRedirect('/users_groups/view_local_users?ack=gid_changed')
