...
from integral_view.core.storage_access.forms import iscsi_stgt_forms
    ...
    try:
        if request.method == "GET":
            ...
            if form.is_valid():
                ...
                url = '/storage_access/view_iscsi_targets?ack=created'
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if form.is_valid():
                ...
                url = '/storage_access/view_iscsi_target?name=%s&ack=lun_created' % target_name
                return django.http.HttpResponseRedirect(url)
            else:
                return_dict["form"] = form
                return django.shortcuts.render_to_response("create_iscsi_lun.html", return_dict, context_instance=django.template.context.RequestContext(request))
            ...
            if form.is_valid():
                ...
                if cd['authentication_type'] == 'incoming':
                    ...
                    audit_str = "Added ISCSI target authentication user %s for target %s" % (
                        cd["username"], cd['target_name'])
                    url = '/storage_access/view_iscsi_target?name=%s&ack=added_target_authentication' % target_name
        ...
        if request.method == "GET":
            ...
            if authentication_type == 'incoming':
                ...
                audit_str = "Removed ISCSI target authentication user %s for target %s" % (
                    username, target_name)
                url = '/storage_access/view_iscsi_target?name=%s&ack=removed_target_authentication' % target_name
