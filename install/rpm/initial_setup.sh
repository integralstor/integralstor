...
### Enable fractalio repository
echo "Enabling fractalio repository..."
yum-config-manager --enable fractalio

if [ $? -ne 0 ]; then
    echo "CRITICAL: Enabling fractalio repository Failed!"
    exit 1
else
    echo "Enabling fractalio repository...Done"
fi
...
if [ $? -ne 0 ]; then
    echo "CRITICAL: Installing 'kernel spaecific kernel-headers and kernel-devel' Failed!"
    exit 1
else
...
yum-config-manager --enable base epel zfs fractalio
