...
def main():
    ...
    try:
        ...
        logger.log_or_print('Current config archive generation initiated.', lg, level='info')
        ...
        if err:
            raise Exception(err)
        config_file_list = [('/etc/samba/smb.conf', 'smb.conf'), ('/etc/krb5.conf', 'krb5.conf'), (db_path, 'integral_view_config.db'), ('/etc/exports', 'exports'), ('/etc/vsftpd/vsftpd.conf', 'vsftpd.conf'), ('/etc/tgt/targets.conf', 'targets.conf'), ('/etc/resolv.conf', 'resolv.conf'), ('/etc/hosts', 'hosts'), ('/etc/passwd', 'passwd'), ('/etc/group', 'group')]
            ...
            for entry in config_dir_list:
                if os.path.exists(entry[0]):
                    ...
                    for root, dirs, files in os.walk(path):
                        ...
                        for file in files:
                            if base:
                                zf.write(os.path.join(root, file), '%s/%s/%s'%(entry[1],base,file))
                            else:
                                zf.write(os.path.join(root, file), '%s/%s'%(entry[1],file))
            zf.close()
