...
def _generate_share_section(f, share_name, workgroup, path, read_only, browseable, comment, auth_method):
  try:
    if not f:
      raise Exception('No file handle passed')
    ...
    if err:
      raise Exception(err)
    ret, err = _generate_unicell_specific_share_body(f, workgroup, path, read_only, browseable, comment, auth_method)
    if err:
      raise Exception(err)
    '''
...
def generate_smb_conf():
  try:
    ...
    with open("%s/smb.conf"%smb_conf_path, "w+") as f:
      ...
      if shl:
        for share in shl:
          ret, err = _generate_share_section(f, share["name"], d["workgroup"], share["path"], share["read_only"], share["browseable"], share["comment"], d["security"])
          if err:
            raise Exception(err)
...
def reload_configuration():
  try:
    ...
    if use_salt:
      ...
      if err:
        raise Exception(err)
      ret, err = services_management.change_service_status('winbind', 'restart')
      if err:
        raise Exception(err)
  except Exception, e:
