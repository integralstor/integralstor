  ...
  try:
    ...
    for g in all:
      if not get_system_groups:
        if g.gr_gid < 501 or g.gr_gid >= 65534:
          continue
      d = {}
      d['grpname'] = g.gr_name
      d['gid'] = g.gr_gid
  ...
  try:
    ...
    for user in all:
      if not get_system_users:
        if user.pw_uid < 501 or user.pw_uid >= 65534:
          continue
...
def set_group_membership(grpname, member_users):
  #Given a group name and a set of users who should be its members, modify the system setup to reflect this.
  try:
    ...
    for user in all_users:
      ...
      new_groups.append(user['grpname']) #primary group always needs to be there
      if grpname not in existing_groups:
        ...
        #This user is part of the group currently so check if (s)he is being removed
      ...
      if modified :
        cmd = 'usermod -G %s %s'%(','.join(new_groups), user['username'])
        print cmd
