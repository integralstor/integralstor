...
def add_user_authentication(target_name, authentication_type, username,password): 
  try:
    ...
    ret, rc = command.execute_with_rc(cmd1)
    if rc != 0:
      err = ''
      tl = command.get_output_list(ret)
      if tl:
    ...
    ret, rc = command.execute_with_rc(cmd2)
    if rc != 0:
      err = ''
      tl = command.get_output_list(ret)
      if tl:
...
def main():
  #targets, err = del_lun('com.integralstor.integralstor-unicell:anand.test.target','/dev/zvol/pool1/disk1') 
  #targets, err = get_targets()
