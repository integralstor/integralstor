import shutil, os
from integralstor_common.common import get_or_create_dir
from integralstor_common import common, scheduler_utils, services_management
...
def generate_exports(share_list):
  try:
        ...
        if 'clients' in share:
          ...
          for client in share['clients']:
            ...
            if 'options' in client:
              client_str = '%s(%s)'%(client_str.strip(), ','.join(client['options']))
    ...
    ret, err = services_management.change_service_status('nfs', 'reload')
    if err:
      raise Exception(err)
  except Exception, e:
...
'''
Not needed now as we are using the services management functionality
def schedule_nfs_reload():
  ret = False
  try:
    db_path, err = common.get_db_path()
    if err:
      raise Exception(err)
    r, err = scheduler_utils.schedule_a_job(db_path, 'Reload NFS service', [{'Reload NFS services':'service nfs reload'}])
    if err:
      raise Exception(err)
    ret = r
  except Exception, e:
    return False, 'Error scheduling NFS service reload : %s'%str(e)
  else:
    return True, None
'''
