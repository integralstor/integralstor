...
import platform
import pprint
from integralstor import services_management, disks, zfs, networking, config, command, system_info
...
def generate_manifest_info(rescan_for_disks=False):
    ...
    try:
        ...
        if not uuid:
            # No uuid, generate
            uuid, err = system_info.set_integralstor_uuid()
            if err:
                raise Exception(err)
        manifest_dict['integralstor_uuid'] = uuid
