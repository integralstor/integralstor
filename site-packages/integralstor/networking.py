    ...
    try:
        ...
        for iface in interfaces.keys():
            ...
            if not interfaces[iface]['addresses']['is_sysd_ip4']:
                error_list.append(
                    'Interface %s is not part of NetworkManager' % iface)
                continue
        ...
        if is_delete == True:
            ...
            error_list.append(
                'Interface %s is not available; did not delete. If the interface is down, please retry after bringing it up.' % if_name)
        if error_list:
            raise Exception(str(error_list))
...
def update_interface_ip(if_name, d):
    try:
        ...
        if init_type == 'systemd':
            ...
            if d['addr_type'] == 'dhcp':
                # Reset the previous IP value to prevent NM from holding it as
                # a secondary IP
