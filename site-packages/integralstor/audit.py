...
from integralstor import event_notifications, datetime_utils, networking, config, db
...
def _parse_audit_entry(entry):
    ...
    try:
        integralstor_action_dict = {
            ...
            "rename_afp_volume": "AFP volume renamed",
            "delete_afp_volume": "AFP volume deleted",
            "create_afp_volume": "AFP volume created",
            "application_action": "Application action performed",
...
def generate_audit_email_body(audit_id):
    ...
    try:
        ...
        if ad_list:
            ad = ad_list[0]
            msg = ' Audit time: %s\n Performed from: %s\n Performed by: %s\n Action type: %s\n Action details: %s.' % (
                ad['time'], ad['ip'], ad['username'], ad['action'], ad['action_str'])
    except Exception, e:
...
if __name__ == "__main__":
    ...
    # print _parse_audit_line('1485243283    192.168.1.79
    # delete_zfs_snapshot       Deleted ZFS snapshot zpool/Unit1@test-dnap-2')
