from integralstor import mail, scheduler_utils, db, config
...
def create_event_notification_trigger(event_type_id, event_subtype_id, subsystem_type_id, notification_type_id, severity_type_id, enc_id, cron_task_id):
    ...
    try:
        ...
        if err:
            raise Exception(err)
        ent_id, err = db.execute_iud(db_path, [["insert into event_notification_triggers (event_type_id, event_subtype_id, subsystem_type_id, notification_type_id, severity_type_id, enc_id, cron_task_id) values (?,?,?,?,?,?,?)", (
            event_type_id, event_subtype_id, subsystem_type_id, notification_type_id, severity_type_id, enc_id, cron_task_id,)]], get_rowid=True)
        if err:
            raise Exception(err)
    except Exception, e:
...
def stringify_event_notification(event_type_id, notification_type_id, subsystem_type_id=-1, severity_type_id=-1, event_subtype_id=-1, reference_table_entries=None):
    ...
    try:
        if not reference_table_entries:
            reference_table_entries, err = get_reference_table_entries(
        ...
        event_subtype = None
        for entry in reference_table_entries['reference_event_subtypes']:
            if entry['event_type_id'] == event_type_id and entry['event_subtype_id'] == event_subtype_id:
                event_subtype = entry['description']
        notification = reference_table_entries['reference_notification_types'][int(
