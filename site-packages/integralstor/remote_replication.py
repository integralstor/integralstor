from integralstor import audit, rsync, datetime_utils, scheduler_utils, tasks_utils, db, config, command, networking, system_info, alerts
...
def _add_rsync_remote_replication(remote_replication_id, entries):
    ...
    try:
        ...
        cron_task_id, err = scheduler_utils.create_cron_task(
            cmd, entries['description'], schedule[0], schedule[1], schedule[2], schedule[3], schedule[4], task_type_id=4)
        if err:
            raise Exception(err)
...
def update_remote_replication_user_comment(remote_replication_id, user_comment):
    try:
        if not (remote_replication_id and user_comment):
        ...
        # update user comment
        update_cmd = "update remote_replications set user_comment = '%s' where remote_replication_id = %d" % (
            user_comment, int(remote_replication_id))
        ret, err = db.execute_iud(
            db_path, [[update_cmd], ], get_rowid=True)
        if err:
            raise Exception(err)

    except Exception, e:
        return False, 'Error updating remote replication user comment: %s' % e
    else:
        return True, None
...
def update_rsync_remote_replication_pause_schedule(remote_replication_id, schedule=None):
    try:
        ...
        if schedule and int(pause_cron_task_id) != -1:
            ...
            if err:
                raise Exception(err)
            description = 'pause %s' % rep_cron_tasks[0]['description']
            pause_cron_task_id, err = scheduler_utils.create_cron_task(
                command, description, schedule[0], schedule[1], schedule[2], schedule[3], schedule[4], task_type_id=4)
            if err:
                raise Exception(err)
        ...
        if rr_list and rr_list[0]:
            for idx, rr in enumerate(rr_list, start=1):
                ...
                if task and task[0]:
                    ...
                    rep = {}
                    rep['sno'] = idx
                    rep['description'] = '\n'.join(wrap('[%s]\n%s' % (str(rr['user_comment']).strip(),
                                                                    str(rr['description']).strip()),
                                                        30))
...
def run_rsync_remote_replication(remote_replication_id):
    ...
    try:
        ...
        if is_running is True:
            ...
            retval, err = alerts.record_alerts([{'subsystem_type_id': 7, 'severity_type_id': 3,
                                                    'component': 'Rsync replication task', 'alert_str': audit_str}])
...
def run_zfs_remote_replication(remote_replication_id):
    ...
    try:
        ...
        if is_running is True:
            ...
            audit.audit("task_fail", audit_str, None, system_initiated=True)
            retval, err = alerts.record_alerts([{'subsystem_type_id': 7, 'severity_type_id': 3,
                                                    'component': 'ZFS replication task', 'alert_str': audit_str}])
            raise Exception(audit_str)
