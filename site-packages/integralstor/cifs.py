...
from integralstor_utils import command, config, services_management
from integralstor_utils import cifs as cifs_common
from integralstor import local_users


def _generate_integralstor_specific_global_section(f, d):
    try:
        if not f:
...
def _generate_integralstor_specific_share_body(f, workgroup, path, read_only, browseable, comment, auth_method):
    try:
        if not f:
            raise Exception('No file handle passed')
...
def _generate_share_section(f, share_name, workgroup, path, read_only, browseable, comment, auth_method, extra_share_param_lines=None, hosts_allow=None, hosts_deny=None):
    try:
        if not f:
            raise Exception('No file handle passed')
        ...
        if err:
            raise Exception(err)
        ret, err = _generate_integralstor_specific_share_body(
            f, workgroup, path, read_only, browseable, comment, auth_method)
        if err:
            raise Exception(err)
        ret, err = cifs_common.generate_common_share_body(
            f, workgroup, path, read_only, browseable, comment, extra_share_param_lines, hosts_allow=hosts_allow, hosts_deny=hosts_deny)
        if err:
            raise Exception(err)
