from integralstor_utils import config, db, networking
...
def generate_pdf_report(name=None, date_on=None, date_from=None, date_till=None):
    try:
        ...
        reports_path, err = config.get_urbackup_reports_path()
        if err:
            raise Exception(err)
        ...
        description = 'Host: %s.%s<br/>' % (host_name, domain_name)
