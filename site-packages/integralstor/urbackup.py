from integralstor import config, db, networking, system_info, filesize
...
def get_logs(date_on=None, date_from=None, date_till=None, last_backup_only=False):
    logs = []
    qry = ''
    client_id_list = []
    try:
        ...
        if last_backup_only:
            ...
            client_ids = db.get_multiple_rows(urb_db, qry)
            for client_id in client_ids[0]:
                qry = 'select * from logs where clientid = %d order by id desc limit 1' % (client_id['id'])
                log = db.get_multiple_rows(urb_db, qry)
                if log[0]:
                    logs.append(log[0][0])
...
def compile_report(date_on=None, date_from=None, date_till=None, last_backup_only=False):
    l = []
    try:
        urb_db, err = config.get_urbackup_db_path()
        if err:
        ...
        if clients:
            for client in clients:
                size = filesize.bytes_to_human_readable_size(client['bytes_used_files'])
                disk_space_used.update({client['id']: size[0]})

        client_status = {}
        for client in clients:
            client_status.update({client['id']: client['file_ok']})

        client_names = {}
        for client in clients:
            client_names.update({client['id']: client['name']})
        logs, err = get_logs(last_backup_only=True)
        if err:
            raise Exception(err)
        if logs:
            for idx, log in enumerate(logs, start=1):
                ...
                if log['clientid'] in disk_space_used:
                    d['disk_space_used'] = disk_space_used[log['clientid']]

                if log['clientid'] in client_status:
                    d['status'] = client_status[log['clientid']]
                ...
                l.append(d)
        for tmp_info in l:
                if tmp_info['exit_status'] == 'Backup succeeded' and tmp_info['avg_transfer_speed'] == 'N.A':
                        tmp_info['avg_transfer_speed'] = '0 KBit/s'

        for tmp_info in l:
                if tmp_info['exit_status'] == 'Backup succeeded' and tmp_info['transfer_size'] == 'N.A':
                        tmp_info['transfer_size'] = '0 KB'
...
#        report, err = compile_report(date_on=curr)
        report, err = compile_report(last_backup_only=True)
        if err:
            raise Exception(err)
...
def generate_pdf_report(name=None, date_on=None, date_from=None, date_till=None, last_backup_only=None):
    try:
        ...
        doc = SimpleDocTemplate(report_path, pagesize=A2)
        ...
        sorted_data = sorted(data[1:], key=lambda x: datetime.datetime.strptime(x[4], '%Y-%m-%d %H:%M:%S'))
        sorted_data.insert(0, data[0])
        for si_no, d in enumerate(sorted_data[1:], start=1):
            d[0] = si_no
        data = sorted_data
...
def generate_todays_pdf_report():
    try:
        ...
        report, err = generate_pdf_report(
            name='urbackup-%s' % curr, last_backup_only=True)
        if err:
            raise Exception(err)
...
if __name__ == '__main__':
    ...
    ret, err = generate_pdf_report(last_backup_only=True)
