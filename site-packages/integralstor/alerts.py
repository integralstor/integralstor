...
from integralstor import event_notifications, datetime_utils, config, db
...
def get_alerts(count=-1, start_time=None):
    ...
    try:
        ...
        if not start_time:
            ...
            if set_count:
                query = 'select * from alerts where last_update_time >= %d order by alert_id desc limit %d' % (
                    int(start_time), count)
            else:
                query = 'select * from alerts where last_update_time >= %d order by alert_id desc' % int(
                    start_time)
        # print query
...
def export_old_alerts(older_than_days=1):
    """Move all alerts older than the older_than_days into a file in /var/log/integralstor/logs/exported_logs dir
