...
def get_alerts(count = -1, start_time = None):
    ...
    try:
        ...
        if not start_time:
            ...
            if set_count:
                query = 'select * from alerts where last_update_time >= %d order by alert_id desc limit %d'%(int(start_time), count)
            else:
                query = 'select * from alerts where last_update_time >= %d order by alert_id desc'%int(start_time)
        #print query
