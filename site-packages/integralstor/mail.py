...
from integralstor import datetime_utils, config, db
...
def send_mail(server, port, username, pswd, tls, rcpt_list, header, body, attachment_location_list=None):
    ...
    try:
        #msg = MIMEText(body)
        msg = MIMEMultipart()
        msg['from'] = 'IntegralSTOR alerting system'
        ...
        if ',' in rcpt_list:
            ...
            emails = rcpt_list.split(' ')
        #to = ','.join(emails)
        to = email.utils.COMMASPACE.join(emails)
        # print to
        ...
        # body, emails)
        for file_location in attachment_location_list or []:
...
def main():
    ...
    # print send_mail(d['server'], d['port'], d['username'], d['pswd'],
    # d['tls'], d['rcpt_list'], "A Test Header", "Hello World")
    pass
