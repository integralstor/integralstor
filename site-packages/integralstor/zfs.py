...
def get_all_datasets():
    ...
    try:
        ...
        return None, 'Error retrieving all datasets : %s' % str(e)
    else:
        return datasets, None
...
def create_zfs_cron_task(cron_task_id, pool_name, ds_zvol_name=None):
    zfs_cron_task_id = -1
    try:
        db_path, err = config.get_db_path()
        if err:
            raise Exception(err)
        if ds_zvol_name:
            cmd = ['insert into zfs_cron_tasks(cron_task_id, pool_name, ds_zvol_name) values (?,?,?)', (cron_task_id, pool_name, ds_zvol_name)]
        else:
            cmd = ['insert into zfs_cron_tasks(cron_task_id, pool_name) values (?,?)', (cron_task_id, pool_name)]
        zfs_cron_task_id, err = db.execute_iud(db_path, [cmd], get_rowid=True)
        if err:
            raise Exception(err)
    except Exception, e:
        return -1, 'Error creating zfs cron task entry: %s ' % str(e)
    else:
        return zfs_cron_task_id, None


def get_zfs_cron_tasks(pool_name=None, zfs_cron_task_id=None):
    zfs_cron_tasks = None
    try:
        db_path, err = config.get_db_path()
        if err:
        ...
        if pool_name:
            if zfs_cron_task_id:
                query = 'select * from zfs_cron_tasks where pool_name="%s" and cron_task_id="%d"' % int(cron_task_id)
            else:
                query = 'select * from zfs_cron_tasks where pool_name="%s"' % pool_name
        elif zfs_cron_task_id:
            query = 'select * from zfs_cron_tasks where zfs_cron_task_id="%d"' % zfs_cron_task_id
        else:
            query = 'select * from zfs_cron_tasks'
        zfs_cron_tasks, err = db.get_multiple_rows(db_path, query)
        if err:
            raise Exception(err)
    except Exception, e:
        return None, 'Error getting ZFS related cron tasks: %s ' % str(e)
    else:
        return zfs_cron_tasks, None
...
def get_pool_events_history(pool_name, action):
    lines = None
    try:
        if not pool_name or not action:
            raise Exception(
                'Invalid request. Pool name and action are required')
        if action not in ['events', 'history']:
            raise Exception('Invalid action')
        if action == 'events':
            cmd = 'zpool events -Hv %s' % pool_name
        elif action == 'history':
            cmd = 'zpool history -i %s' % pool_name
        lines, err = command.get_command_output(cmd)
        if err:
            raise Exception(err)
    except Exception, e:
        return None, "Error retrieving ZFS pool events/history : %s" % str(e)
    else:
        return lines, None
