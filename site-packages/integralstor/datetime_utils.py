...
def get_epoch(when='now', num_previous_days=0):
    ...
    try:
        ...
        if when == 'now':
            ...
            reqd_midnight = midnight - datetime.timedelta(days=num_previous_days)
...
def main():
    ...
    print dt, err
    '''
    dt, err = convert_from_epoch(ep, return_format='str', str_format = '%c', to='utc')
    print 'utc', dt, err
    '''
    dt, err = convert_from_epoch(ep, return_format='datetime', str_format = '%c', to='local')
    print dt, err
    '''
    dt, err = convert_from_epoch(ep, return_format='str', str_format = '%c', to='local')
    print 'local', dt, err
