...
def execute(cmd=None, shell=False):
    ...
    try:
        ...
        env, err = datetime_utils.get_tz_updated_env()
        if err:
            raise Exception(err)
    ...
    try:
        ...
        if run_as_user_name is not None:
            ...
            proc = subprocess.Popen(comm_list, preexec_fn=_demote(
                user_uid, user_gid), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=shell, env=env)
        else:
            proc = subprocess.Popen(
                comm_list, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=shell, env=env)
        ret = proc.communicate()
...
def execute_with_conf(cmd=None, response='y'):
    ...
    try:
        ...
        proc = subprocess.Popen(
            comm_list, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
