...
import urllib
import json
from integralstor_utils import config, networking
...
def _generate_servers_config_file(server_dict):
    try:
        server_config_file_path, err = _get_config_file_path()
        if err:
            raise Exception(err)
    ...
    try:
        ...
        with open(server_config_file_path, 'r') as f:
            try:
                ...
                servers = {}
                with open(server_config_file_path, 'w') as wfd:
                    json.dump(servers, wfd, indent=2)
    except Exception, e:
...
def get_status(ip):
    ...
    try:
        ...
        status['ip'] = ip
        url = '%s://%s/api_get_status' % (servers[ip]['prefix'], ip)
        response = urllib.urlopen(url)
        output = json.loads(response.read())
        if not output:
            raise Exception('Invalid response from the server')
        data = output[output.keys()[0]]
        pools = []
        if 'pools' in data:
        ...
        if 'services' in data:
            for service_name, service_info in data['services'].items():
                ...
                service_dict['status_str'] = service_info[1]
