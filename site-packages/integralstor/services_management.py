...
def get_services_list():
    services = [('ntpd', 'NTP'), ('smb', 'CIFS - smb'), ('winbind', 'CIFS - winbind'), ('tgtd', 'ISCSI'),
                ('nfs', 'NFS'), ('vsftpd', 'FTP'), ('shellinaboxd', 'Shell Access'), ('urbackup-server', 'UrBackup Server')]
    if services:
...
def update_service_status(service, action):
    out = None
    try:
        ...
        if init_type == 'init':
            ...
            raise Exception(
                'No systemd or init found. Not changing the service status')
        # print cmd

        # if init_type == 'init':
        with open('/tmp/cout', 'w') as f:
            proc = subprocess.Popen(
                cmd.split(), stdout=f, stderr=f, close_fds=True, universal_newlines=True)
            proc.wait()
